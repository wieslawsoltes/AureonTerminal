...
import {readFile,writeFile,chmod,open,link,rm} from 'node:fs/promises';
...
export class PrivateVault {
  ...
  static async open(directory){
    const file=join(directory,'vault.key');let key;
    try{key=await readFile(file);}catch(error){
      if(error.code!=='ENOENT')throw error;
      const temporary=file+'.'+randomBytes(12).toString('hex')+'.tmp',handle=await open(temporary,'wx',0o600);
      try{await handle.writeFile(randomBytes(32));await handle.sync();}finally{await handle.close();}
      try{try{await link(temporary,file);}catch(e){if(e.code!=='EEXIST')throw e;}key=await readFile(file);}
      finally{await rm(temporary,{force:true});}
    }
    await chmod(file,0o600);return new PrivateVault(key);
  }
