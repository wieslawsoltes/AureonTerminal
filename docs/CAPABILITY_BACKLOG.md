...
serialization and regression evidence are present. CI passes do not provision
external data rights, identity infrastructure or physical devices.
...
| Multi-host private services | A transactional network-store adapter with fenced leases, authorization and event replay; independently killed processes across hosts; operator-controlled secrets and deployment. |
| Account recovery and ownership | Verified delivery/ownership and audited recovery flows with enumeration/rate-limit defenses; operator configuration and recipient acceptance. |
| Native application packaging | Reproducible signed desktop/mobile builds, storage/lifecycle integration and actual platform smoke tests; do not count host-project skeletons as released apps. |
| Physical GPU coverage | Renderer fixtures on real AMD/Intel/NVIDIA/Apple devices, device-loss/recovery, measured CPU/GPU timings, memory and workload documentation. |

The 4.4 round closes explicit timezone/session rules, closed-source resampling,
causal session overlays, snapshot lifecycle and gap-preserving line presentation.
It does not claim a maintained global holiday service, options/venue entitlement,
