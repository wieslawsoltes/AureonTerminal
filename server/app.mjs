/** Native Node single-process service: per-user data, CSRF-protected writes,
...
export async function createAureonServer({root=resolve('.'),dataDir=resolve(root,'.aureon-data'),origin='',fetcher=fetch,key='',secret='',feed='iex',brokerUser='',monitor=true,monitorInterval=15000,registration=true}={}){
  root=await realpath(root);const store=await new Store(dataDir).init(),providers=new Providers({fetcher,key,secret,feed}),clients=new Map(),rate=new Map();let server;
  const emit=(userId,event)=>{const data='data: '+JSON.stringify(event)+'\n\n';for(const[res,client]of clients){if(Date.now()>client.expires){res.end();clients.delete(res);continue;}if(userId==null||userId===client.userId)res.write(data);}};
  const alertMonitor=new AlertMonitor(store,providers,emit,{interval:monitorInterval});
  const throttle=(id,max=120,window=60000)=>{const now=Date.now();let r=rate.get(id);if(!r||r.until<now){r={count:0,until:now+window};rate.set(id,r);}if(++r.count>max)throw problem(429,'Too many requests; retry later.');if(rate.size>10000)for(const[k,v]of rate)if(v.until<now)rate.delete(k);};
  ...
  async function handle(req,res){
    try{
      ...
      if(url.pathname.startsWith('/v2/')){
        throttle(req.socket.remoteAddress+':api',600);
        const path=url.pathname.slice(4).replace(/\/$/,''),auth=getSession(req),user=auth?.user;
        if(method==='GET'&&path==='session'){json(res,200,{user:user?{id:user.id,username:user.username}:null,csrf:auth?.session.csrf||null,capabilities:{broker:brokerAllowed(user),monitor,registration}});return;}
        if(['register','login'].includes(path)&&method==='POST'){
          ...
          const raw=token(),session={tokenHash:sha(raw),userId:account.id,csrf:token(),expires:Date.now()+SESSION_MS};await store.transaction(s=>{s.sessions=s.sessions.filter(x=>x.expires>Date.now());const own=s.sessions.filter(x=>x.userId===account.id);if(own.length>=10){const remove=new Set(own.slice(0,own.length-9).map(x=>x.tokenHash));s.sessions=s.sessions.filter(x=>!remove.has(x.tokenHash));}s.sessions.push(session);});res.setHeader('Set-Cookie',cookie(raw,SESSION_MS/1000,expected.startsWith('https:')));json(res,200,{ok:true,user:{id:account.id,username:account.username}});return;
        }
        if(!user)throw problem(401,'Sign in required.');
        if(!['GET','HEAD'].includes(method)){if(req.headers['x-csrf-token']!==auth.session.csrf)throw problem(403,'Invalid CSRF token.');throttle(user.id+':writes',120);}
        ...
        if(path==='events'&&method==='GET'){
          if([...clients.values()].filter(c=>c.userId===user.id).length>=5)throw problem(429,'Maximum five event streams per user.');res.writeHead(200,{'Content-Type':'text/event-stream','Cache-Control':'no-cache','Connection':'keep-alive'});res.write('data: '+JSON.stringify({type:'connected'})+'\n\n');clients.set(res,{userId:user.id,expires:auth.session.expires,sessionHash:auth.session.tokenHash});const keepAlive=setInterval(()=>{if(Date.now()>auth.session.expires)res.end();else res.write(': heartbeat\n\n');},15000);req.on('close',()=>{clearInterval(keepAlive);clients.delete(res);});return;
        }
        ...
        if(path==='alerts'){
          ...
          if(method==='POST'){
            const raw=await body(req,10000);validateAlert(raw);if(!/^[A-Z0-9.-]+-USD$/.test(raw.symbol)||![60,300,900,3600,14400,21600,86400,604800].includes(raw.interval))throw problem(400,'Server monitor requires a supported Coinbase USD product and interval.');const allowed=new Set(['price','close','volume','rsi','ema','sma']);for(const c of raw.conditions)if(!allowed.has(c.source)||(typeof c.target==='string'&&!allowed.has(c.target)))throw problem(400,'Server supports price, close, volume, RSI14, EMA20 and SMA50 conditions. Custom scripts remain browser-local.');const r={id:randomUUID(),userId:user.id,symbol:raw.symbol,interval:raw.interval,conditions:raw.conditions,combine:raw.combine||'all',frequency:raw.frequency||'once',cooldown:raw.cooldown||0,expires:raw.expires??null,message:String(raw.message||''),active:true,mode:'market',created:Date.now(),updated:Date.now()};await store.transaction(s=>{if(s.alerts.filter(r=>r.userId===user.id).length>=50)throw problem(507,'Maximum 50 server alerts.');s.alerts.push(r);});json(res,201,r);return;
          }
        }
        ...
        if(path==='ideas'){
          if(method==='GET'){json(res,200,{items:store.state.ideas.slice(-100).reverse().map(({workspace,...i})=>i)});return;}
          if(method==='POST'){const raw=await body(req,15_000_000),title=string(raw.title,'idea title',200),text=string(raw.text,'idea body',10000),symbol=string(raw.symbol,'symbol',40);const idea={id:randomUUID(),userId:user.id,username:user.username,title,text,symbol,workspace:raw.workspace?validatePayload(raw.workspace):null,created:Date.now(),likes:[],comments:[]};await store.transaction(s=>{if(s.ideas.length>=5000)throw problem(507,'Server idea quota exceeded.');s.ideas.push(idea);});emit(null,{type:'idea',id:idea.id});json(res,201,{id:idea.id});return;}
        }
        const ideaMatch=/^ideas\/([^/]+)(?:\/(like|comments))?$/.exec(path);
        if(ideaMatch){const[id,operation]=ideaMatch.slice(1);if(method==='GET'&&!operation){const idea=store.state.ideas.find(i=>i.id===id);if(!idea)throw problem(404,'Idea not found.');json(res,200,idea);return;}if(method==='DELETE'&&!operation){await store.transaction(s=>{const i=s.ideas.find(i=>i.id===id&&i.userId===user.id);if(!i)throw problem(404,'Idea not found.');s.ideas=s.ideas.filter(x=>x!==i);});emit(null,{type:'idea',id,deleted:true});json(res,200,{ok:true});return;}if(method==='POST'&&operation){const raw=await body(req,3000);await store.transaction(s=>{const idea=s.ideas.find(i=>i.id===id);if(!idea)throw problem(404,'Idea not found.');if(operation==='like'){const pos=idea.likes.indexOf(user.id);pos<0?idea.likes.push(user.id):idea.likes.splice(pos,1);}else{if(idea.comments.length>=500)throw problem(507,'Comment limit reached.');idea.comments.push({id:randomUUID(),userId:user.id,username:user.username,text:string(raw.text,'comment',2000),created:Date.now()});}});emit(null,{type:'idea',id});json(res,200,{ok:true});return;}}
        if(path.startsWith('broker/')||path==='market/bars'){
          if(!brokerAllowed(user))throw problem(403,'Paper brokerage/data access is not enabled for this user.');throttle(user.id+':provider',60);
      ...
      if(url.pathname.startsWith('/api/')){throttle(req.socket.remoteAddress+':market',120);const data=await providers.coinbase(url.pathname.slice(4)+url.search);json(res,200,data);return;}
  ...
  if(monitor)alertMonitor.start();
  return{server,store,providers,alertMonitor,emit,async close(){await alertMonitor.stop();for(const res of clients.keys())res.end();clients.clear();server.closeIdleConnections?.();if(server.listening)await new Promise((resolve,reject)=>server.close(e=>e?reject(e):resolve()));await store.flush();}};
}
