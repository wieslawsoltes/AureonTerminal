...
export function prepareTradeProfiles(raw,interval,options={}){const trades=validateTrades(raw),tickSize=options.tickSize??Math.max(1e-8,(trades.at(-1)?.price||1)*.0001);return{footprint:new Map(footprint(trades,{interval,tickSize}).map(b=>[b.t,b])),tpo:tpoFromTrades(trades,{tickSize,periodSeconds:options.tpoPeriod??1800}),tickSize,periodSeconds:options.tpoPeriod??1800};}
...
export function renderProBar(chart,{b,x,width,yo,yc,yh,yl,col,volume,maxVolume,previous}){
  ...
  for(const level of fp.levels){const y=chart.toY(level.price),row=Math.max(1,Math.abs(chart.toY(level.price+tickSize/2)-chart.toY(level.price-tickSize/2))*.94);if(y<box.y-row||y>box.y+box.h+row)continue;
    ...
    if(w>=85&&cell>=13){chart.labels.push({x:x-4,y,text:shortSize(level.sell),align:'right',color:colors.bright},{x:x+4,y,text:shortSize(level.buy),color:colors.bright});}
  }
...
export function renderTradeOverlay(chart){
  ...
  chart.labels.push({x:12,y:chart.price.y+15,text:!profiles?'Actual-trade data required · import in Pro tools or connect Order Flow':chart.style==='footprint'?'Observed sell × buy · gray = unknown aggressor · no inferred executions':'Observed-price TPO · letters are UTC periods; unobserved prices stay empty',color:colors.text});
