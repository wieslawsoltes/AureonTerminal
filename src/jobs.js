...
export function performJob(type,bars,options={}){switch(type){case'studies':return computeStudies(bars,options.specs);case'script':return runScript(options.source,bars,options);case'backtest':{const commands=options.source?runScript(options.source,bars,options).commands:movingAverageSignals(bars,options.fast,options.slow);return runBacktest(bars,{...options,commands});}case'sweep':return parameterSweep(bars,movingAverageSignals,options);default:throw new Error('Unknown job '+type);}}
