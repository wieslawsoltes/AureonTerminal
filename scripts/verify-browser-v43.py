    ...
    with tempfile.TemporaryDirectory(prefix='aureon-v43-') as private:
        ...
        try:
            async with async_playwright() as p:
                ...
                async def device():
                    nonlocal adapter
                    result=await page.evaluate('''async(expectedVersion)=>{const r=aureon.app.chart.renderer;await r.ready;aureon.app.chart.draw();if(r.mode!=='WebGPU')throw Error(r.reason);if(!r.diagnostics().stats.gpuFrames)throw Error('No actual GPU frame');const info=r.diagnostics();if(info.adapter.fallback!==true&&!/swiftshader/i.test(JSON.stringify(info.adapter)))throw Error('Expected fallback-adapter signal or software identity: '+JSON.stringify(info.adapter));if(document.querySelector('.version').textContent!=='v'+aureon.releaseVersion||aureon.releaseVersion!==expectedVersion)throw Error('Release identity differs');return info;}''',json.loads((ROOT/'package.json').read_text())['version'])
