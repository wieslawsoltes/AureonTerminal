...
export async function createAureonServer({root=resolve('.'),dataDir=resolve(root,'.aureon-data'),origin='',fetcher=fetch,key='',secret='',feed='iex',brokerUser='',monitor=true,monitorInterval=15000,registration=true,pro={}}={}){
  ...
  const emit=(userId,event)=>{const data='data: '+JSON.stringify(event)+'\n\n';for(const[res,client]of clients){if(Date.now()>client.expires||!store.state.sessions.some(s=>s.tokenHash===client.sessionHash)){res.end();clients.delete(res);continue;}if(userId==null||userId===client.userId)res.write(data);}};
  const proServices=await new ProServices(store,providers,emit,{...pro,fetcher,monitorInterval}).init();
  ...
  async function handle(req,res){
    try{
      ...
      if(url.pathname.startsWith('/v2/')){
        ...
        if(path.startsWith('pro/security/'))throttle(user.id+':security',20);if(path==='pro/scripts/run')throttle(user.id+':scripts',20);if(path.startsWith('pro/research/'))throttle(user.id+':research',30);if(path.startsWith('pro/live/'))throttle(user.id+':live',20);
