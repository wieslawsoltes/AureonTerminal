 ...
 with tempfile.TemporaryDirectory(prefix='aureon-browser-') as private:
  ...
  try:
   async with async_playwright() as p:
    ...
    async def wait_js(predicate, timeout=8.0):
     """Poll via DevTools, not an injected eval loop forbidden by the app CSP."""
     deadline=time.monotonic()+timeout
     while True:
      if await page.evaluate(predicate):return
      if time.monotonic()>=deadline:
       raise TimeoutError(f'Browser condition did not become true: {predicate}')
    ...
    try:await wait_js('() => globalThis.aureon?.workbench && aureon.app.series.bars.length===1000')
    except Exception:
    ...
    async def analytics():
     ...
     await wait_js('() => aureon.workbench.pro.results?.price>0')
    ...
    async def patterns():
     ...
     await wait_js('() => aureon.workbench.pro.patterns.length>0')
    ...
    async def trades():
     ...
     await wait_js('() => aureon.workbench.observedTrades?.length===160')
    ...
    async def tiles():
     await js('async()=>{await aureon.workbench.tiles.create(16);}');await wait_js('() => aureon.workbench.extraCharts.length===15&&aureon.workbench.extraCharts.every(t=>t.chart.bars.length>0)')
    ...
    await check('Profiler and entry-ID portfolio strategy execute with live adapter unavailable',lambda:js("async()=>{const w=aureon.workbench;w.currentScript().source=['strategy(\"Fixture\")','if bar_index == 1','    strategy.entry(\"one\", strategy.long, qty=1)','if bar_index == 10','    strategy.close(\"one\")'].join(String.fromCharCode(10));await w.runScript(false);if(!w.scriptResult?.profile?.length)throw Error(w.scriptError||'profile');const r=await w.jobs.run('portfolio',w.bars(),{source:w.currentScript().source});if(r.trades.length!==1)throw Error('entry/close');return{profile:w.scriptResult.profile.length,trades:r.trades.length};}"))
    ...
    async def pwa():
     await js('()=>aureon.workbench.pro.registerPWA()');await wait_js('() => navigator.serviceWorker.controller!==null')
     ...
     await context.set_offline(True);await page.goto(base+'/?demo',wait_until='load');await wait_js('() => aureon?.workbench && aureon.app.series.bars.length===1000');state=await js('()=>({kind:aureon.app.kind,worker:!!aureon.workbench.jobs.worker})');assert state['kind']=='demo',state;await context.set_offline(False);return{'cacheEntries':cachekeys,'offline':state}
