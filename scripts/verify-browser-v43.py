...
async def main():
    OUT.mkdir(parents=True,exist_ok=True);tests=[];errors=[];adapter=None;version=None
    with tempfile.TemporaryDirectory(prefix='aureon-v43-') as private:
        ...
        try:
            async with async_playwright() as p:
                ...
                async def device():
                    nonlocal adapter
                    result=await page.evaluate('''async()=>{const r=aureon.app.chart.renderer;await r.ready;aureon.app.chart.draw();if(r.mode!=='WebGPU')throw Error(r.reason);if(!r.diagnostics().stats.gpuFrames)throw Error('No actual GPU frame');const info=r.diagnostics();if(!/swiftshader/i.test(JSON.stringify(info.adapter)))throw Error('Expected explicitly configured software adapter: '+JSON.stringify(info.adapter));return info;}''')
