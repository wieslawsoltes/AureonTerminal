...
const app=await createAureonServer({root,storage:process.env.AUREON_STORAGE||'json',dataDir:process.env.AUREON_DATA_DIR||resolve(root,'.aureon-data'),origin:process.env.AUREON_ORIGIN||'',key:process.env.APCA_API_KEY_ID||'',secret:process.env.APCA_API_SECRET_KEY||'',feed:process.env.ALPACA_DATA_FEED||'iex',brokerUser:process.env.AUREON_BROKER_USER||'',monitor:process.env.AUREON_MONITOR!=='0',monitorInterval:Number(process.env.AUREON_POLL_MS||15000),registration:process.env.AUREON_REGISTRATION!=='0',pro:{
