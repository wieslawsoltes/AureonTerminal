...
export async function backup(directory,file,passphrase,{storage}={}){
  const path=resolve(directory);let state,sourceStorage='json';
  if(storage!==undefined&&!['json','sqlite'].includes(storage))throw new Error('Unsupported backup storage driver');
  if(storage===undefined){
    const exists=async name=>{try{await stat(join(path,name));return true;}catch(e){if(e.code==='ENOENT')return false;throw e;}};
    const [json,sqlite]=await Promise.all([exists('state.json'),exists('state.sqlite')]);
    if(json&&sqlite)throw new Error('Ambiguous backup source: specify storage json or sqlite explicitly');
    if(!json&&!sqlite)throw new Error('No backup storage source exists');
